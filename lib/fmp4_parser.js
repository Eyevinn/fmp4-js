...
const log = require("logplease").create("fMP4Parser", { useColors: false, showTimestamp: false });
...
function _readUint16(bytes) {
  return Buffer.from(bytes).readUInt16BE(0);
}

function _readUint24(bytes) {
  return Buffer.from([0x00, bytes[0], bytes[1], bytes[2]]).readUInt32BE(0);
}
...
function _readUint64(bytes) {
  let big = new Uint64BE(Buffer.from(bytes));
  return big.toNumber();
}
...
fMP4Parser.prototype._parseBoxes = function _parseBoxes(chunk) {
  ...
  while (pos < len) {
    ...
    log.debug('0x' + Buffer.from(chunk.slice(pos, pos + 4)).toString('hex') + ' 0x' + Buffer.from(chunk.slice(pos + 4, pos + 8)).toString('hex'));
    let size = _readUint32(chunk.slice(pos, pos + 4));
    let type = _read4CC(chunk.slice(pos + 4, pos + 8));

    let largesize;
    if (size === 1) {
      // Actual size is in the field 'largesize'
      largesize = _readUint64(chunk.slice(pos + 8, pos + 16));
      log.debug(`largesize=${largesize}`);
    }

    let box = new MP4Box(largesize ? largesize : size, type, chunk.slice(pos, pos + (largesize ? 16 : 8)));
    log.debug(`size=${size}, type=${type}`);


    if (pos + box.hdr.size > len) {
      // We need more data
      log.debug(`need more data ${pos} + ${box.hdr.size} > ${len}`);
      return len - pos;
    } else {
